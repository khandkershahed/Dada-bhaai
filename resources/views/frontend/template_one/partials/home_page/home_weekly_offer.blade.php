@php
    use Illuminate\Support\Facades\File;
    use Carbon\Carbon;

    $offer_cats = App\Models\Admin\OfferCategory::with('offer')
        ->whereHas('offer')
        ->where('status', '1')
        ->limit(4)
        ->latest()
        ->get();

    $noOfferImage = asset('upload/no-img.jpg'); // fallback image
@endphp

@if (count($offer_cats) > 0)
    <div class="offer-deals">
        <div class="offer--deals__main offer-deals--bg py-30"
            data-background="{{ asset('frontend/template_one/assets/img/bg/offer_banner.jpg') }}">
            <div class="container">
                <div class="row">
                    <div class="col-sm-12">
                        <div class="offer--deals__tabs">
                            <div class="tab-content" id="myTabContent">

                                @foreach ($offer_cats as $key => $offer_cat)
                                    @if (!empty($offer_cat->offer))
                                        @php
                                            $offer = $offer_cat->offer;
                                            // Determine offer image with fallback
                                            if (!empty($offer->offer_image) && File::exists(public_path('storage/offer_image/' . $offer->offer_image))) {
                                                $offerImage = asset('storage/offer_image/' . $offer->offer_image);
                                            } else {
                                                $offerImage = $noOfferImage;
                                            }
                                        @endphp

                                        <div class="tab-pane fade show {{ $key == 0 ? 'active' : '' }}"
                                            id="cat-tab-{{ $offer_cat->id }}" role="tabpanel"
                                            aria-labelledby="home-tab">
                                            <div class="row align-items-center">
                                                <div class="col-xl-4 col-lg-4">
                                                    <h2>{{ $offer_cat->offer_category_name }}</h2>
                                                    <h6 class="mb-3">{{ $offer->name }}</h6>
                                                    <p>Hurry Up Before Offer Will End</p>
                                                </div>
                                                <div class="col-xl-4 col-lg-4">
                                                    <img class="img-fluid w-100"
                                                        src="{{ $offerImage }}"
                                                        alt="Offer Image">
                                                </div>
                                                <div class="col-xl-4 col-lg-4">
                                                    <div class="offer--deals__products mb-30 mt-30">
                                                        <div class="products--deals__content mb-35">
                                                            <h6 class="f-700 mb-20">
                                                                <a href="javascript:void(0)">
                                                                    {{ optional($offer->products)->product_name }}
                                                                </a>
                                                            </h6>
                                                            <span>$ </span>
                                                            <span class="price-old">{{ $offer->price }}</span>
                                                            <span class="f-600 grenadier-color">$<span
                                                                    class="price-new f-600 grenadier-color">{{ $offer->discount_price }}</span></span>
                                                        </div>

                                                        <div class="product-countdown mb-15">
                                                            <div class="time-count-deal">
                                                                <div class="countdown-list"
                                                                    data-countdown="{{ $offer->end_date }}">
                                                                </div>
                                                            </div>
                                                        </div>

                                                        <div class="product--footer__deals">
                                                            @if ($offer->end_date >= Carbon::now())
                                                                <a type="submit" style="cursor:pointer;"
                                                                    data-product_id="{{ optional($offer->products)->id }}"
                                                                    data-discount_price="{{ $offer->discount_price }}"
                                                                    class="add-link f-700 add_to_offer_btn">+ Add To Cart</a>
                                                            @else
                                                                <p class="add-link f-700 add_to_offer_btn">Offer Ended</p>
                                                            @endif
                                                        </div>

                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    @endif
                                @endforeach

                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="offer--deals__menu">
            <div class="container">
                <div class="row">
                    <div class="col-sm-12">
                        <div class="deals--nav__menu">
                            <ul class="nav nav-tabs justify-content-center" id="myTab" role="tablist">
                                @foreach ($offer_cats as $key => $offer_cat)
                                    @if (!empty($offer_cat->offer))
                                        <li class="nav-item">
                                            <a class="nav-link {{ $key == 0 ? 'active' : '' }}"
                                                data-toggle="tab"
                                                href="#cat-tab-{{ $offer_cat->id }}"
                                                role="tab"
                                                aria-controls="cat-tab-one"
                                                aria-selected="{{ $key == 0 ? 'true' : 'false' }}">
                                                {{ $offer_cat->offer_category_name }}
                                            </a>
                                        </li>
                                    @endif
                                @endforeach
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
@endif
